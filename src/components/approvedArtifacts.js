...
export default function ViewFiles() {
  ...
  async function loadfileNFT() {
    ...
    const data = await contract.fetchArtifactsApproved();
    /*
  ...
  async function ArticleDetails(nft) {
  ...
  if (loadingState === "loaded" && !nfts.length) {
    return (
      <div sx={styles.section}>
        <h1 className="px-20 py-10 text-3xl text-white">Empty space, no approved scientific artifacts yet</h1>
      </div>
    );
  }
  return (
    <Box as="section"  sx={styles.section}>
      <div className="bg-blue-100 text-xl text-center text-black font-bold pt-5 pb-4">
        <h1> Approved Scientific Artifacts</h1>
      </div>
          ...
          {nfts.map((nft, i) => (
            ...
            <div key={i} className="shadow rounded-xl overflow-hidden border-2 border-white-500">
              ...
              <div className="p-2 bg-black">
                <button type="button" className="mt-1 w-full bg-blue-500 text-white font-bold py-2 px-12 rounded" onClick={() => ArticleDetails(nft)}>Get Artifact</button>
              </div>
